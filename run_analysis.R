run_analysis <- function(){
        ...
        # Import libraries that will be used in thes cript.
        library(data.table)
        ...
        # Question 1. Merge the training and test sets to create one dataset.
        ...
        # NOTE: As I will be using this data grep, data.frame is easier to utilise in this case compared to data.table.
        featureFile <- file.path(dataFolder, "features.txt")
        feature_data <- read.table(featureFile, colClasses=c("numeric", "character"))
        
        # Exctract names of features with mean() or std(). This may be useful in subsequent questions.
        feature_names <- grep("mean\\(\\)|std\\(\\)", feature_data[, 2], value=TRUE)
        
        # Extract positions of above names. The +2 will be needed because the first two columns
        ...
        # Index the look-up vector 'activity_lookup' with the vector (column 3 of 'data_subset')
        ...
        # Create vector of column names using 'feature_names' extraced at Question 2.
        ...
        # Question 5. From the dataset in step 4, create a second, idipenedent tidy dataset 
        # with the average of each variable for each activity and each subject.
        data_summary <- data_subset %>% group_by(activity_id, subject_id) %>%
                summarise_all(mean, na.rm=TRUE)
        
        
        # Return dataset from question 5.
        data_summary
}